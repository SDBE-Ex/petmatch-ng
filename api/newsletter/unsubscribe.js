...
// Public, unauthenticated GET so a one-click unsubscribe link in the
// weekly newsletter works with no login — same pattern as
// api/digest/unsubscribe.js: the security boundary is knowing the
// per-subscriber unsubscribe_token itself (an unguessable uuid), not a
// session. Needs the service-role key because subscribers has no
// anon/authenticated SELECT or UPDATE policy at all (admin-only, see
// 20260731213000_subscribers.sql).
...
function unsubscribePage(success) {
  const message = success
    ? "You're unsubscribed from PetMatch news. You won't get the weekly post summary anymore."
    : "That link didn't work. It may have already been used.";
  return `<!DOCTYPE html>
